...
def test_calculateHandlen():
    ...
    failure = False
    # dictionary of hands
    hands = [({'a':1, 'q':1, 'l':2, 'm':1, 'u':1, 'i':1}, 7),
             ({},0)
             ]
    for hand in hands:
        handLen = calculateHandlen(hand[0])
        if handLen != hand[1]:
            print("FAILURE: test_calculateHandlen()")
            print("\tExpected", hand[1],
                  "length but got '" + str(handLen) + "' for hand '" + str(hand[0]))
            failure = True
    if not failure:
...
def test_playHand(wordList):
    ...
    print("SUCCESS: test_playHand(wordList)")
